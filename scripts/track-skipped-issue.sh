...
ISSUE_BODY=$(echo "$MANUAL_SKIPS" | jq -r '
  ...
  def entry:
    ...
    + (if (.severity // .max_severity) then "\n  - Severity: **\(.severity // .max_severity)**" else "" end)
  ...
  "The following Patchstack-reported vulnerabilities need manual attention "
  + "— they cannot be auto-fixed by the weekly Composer update.\n\n"
  + (group_by(category) | map(
       section_intro(.[0] | category) + "\n"
       + (map(entry) | join("\n"))
     ) | join("\n\n"))
  + "\n\n---\n\nAuto-generated by `.github/workflows/vulnerability-check.yml`. "
  + "Will auto-close once all entries are resolved.\n"
')
